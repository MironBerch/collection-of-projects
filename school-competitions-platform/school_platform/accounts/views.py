...
class PersonalInfoEditView(
    ...
    profile_form: StudentProfileForm | TeacherProfileForm = None
    user_info_form: UserInfoForm = None

    def dispatch(self, request: HttpRequest, *args, **kwargs):
        if (
            request.user.profile.is_a_student and
            request.user.profile.from_current_school
        ):
            self.profile_form = StudentProfileForm(
                data=request.POST or None,
                instance=request.user.profile,
                exclude_year_of_study=True,
                exclude_school=True,
            )
        elif request.user.profile.is_a_student:
            self.profile_form = StudentProfileForm(
                data=request.POST or None,
                instance=request.user.profile,
                exclude_school_class=True,
            )
        elif (
            not request.user.profile.is_a_student and
            not request.user.profile.from_current_school
        ):
            self.profile_form = TeacherProfileForm(
                data=request.POST or None,
                instance=request.user.profile,
                exclude_school=True,
            )
