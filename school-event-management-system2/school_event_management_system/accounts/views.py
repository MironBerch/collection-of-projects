...
from school.services import set_new_class_teacher_for_school_class
...
class PersonalInfoEditView(
    ...
    def post(self, request: HttpRequest, *args, **kwargs):
        if self.user_info_form.is_valid() and self.profile_form.is_valid():
            ...
            # if teacher has been changed school class
            if (
                'school_class' in self.profile_form.changed_data and
                request.user.role == 'учитель'
            ):
                set_new_class_teacher_for_school_class(
                    user=request.user,
                    school_class=self.profile_form.cleaned_data['school_class'],
                )
